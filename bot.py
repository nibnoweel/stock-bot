...
# ────────────────────────────────────────────────
# 통합 스캔 + PDF 전송 (기존 구조 유지 + 점수/테마 추가)
# ────────────────────────────────────────────────
async def run_full_scan(context: ContextTypes.DEFAULT_TYPE = None, bot: Bot = None):
    ...
    try:
        ...
        # 4. 신규: 점수 스캔 (조건 통과 종목에 한해 수급+점수 계산)
        scores = None
        if stock_results:
        ...
        if themes:
            top3 = themes[:3]
            theme_lines = "\n\n🔥 *오늘의 테마*\n" + "\n".join(
                f"  {t.direction} {t.theme_name} ({t.news_count}건)" for t in top3
            )
    ...
    try:
        ...
        for i, t in enumerate(themes[:8], 1):
            ...
            lines.append(
                ...
                f"  섹터: {sectors}  |  뉴스: {t.news_count}건\n"
            )
